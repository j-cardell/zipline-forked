...
export const fileShares = pgTable(
  ...
  (table) => [
    primaryKey({ name: 'file_share_pkey', columns: [table.id] }),
    uniqueIndex('file_share_token_key').on(table.token),
    index('file_share_file_id_idx').on(table.fileId),
    index('file_share_token_idx').on(table.token),
  ],
);
